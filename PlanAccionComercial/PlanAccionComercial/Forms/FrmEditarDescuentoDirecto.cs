...
{
	public partial class FrmEditarDescuentoDirecto : Form
	{
		// int dias = 0;
		...
		public string consecutivo_retro = null;
		...
		public FrmEditarDescuentoDirecto()
		{
			InitializeComponent();
		}
		...
		private string CrearMensajeResponsable(string tipo_comunicacion)
		{
			...
			if (retro == true)
			{
				message += $@"<div>
                                ...
                                <p style=font-size:15px;margin-top:0px;margin-bottom:0px>{consecutivo_retro.PadLeft(4, '0')} - {nombre_dinamica}</p>
		...
		private void btn_cargar_retro_Click(object sender, EventArgs e)
		{
			...
			try
			{
				new FrmCargarDescuentoDirecto(this).ShowDialog(this);
				
				if (!txt_consecutivo.Text.Equals(""))
				{
					int id_descuento = Convert.ToInt32(txt_consecutivo.Text);
					DescuentoDirecto descuento_directo = new DescuentoDirecto();
					List<int> tipo_com = descuento_directo.ObtenerTiposComunicacionDescuento(id_descuento);
					if (tipo_com != null)
						for (int i = 0; i < clb_tipo_comunicacion.Items.Count; i++)
						{
					...
					List<int> tipo_exh = descuento_directo.ObtenerTiposExhibicionDescuento(id_descuento);
					if (tipo_exh != null)
						for (int i = 0; i < clb_tipo_exhibicion.Items.Count; i++)
						{
							clb_tipo_exhibicion.SelectedIndex = i;
							for (int j = 0; j < tipo_exh.Count; j++)
							{
								if (Convert.ToInt32(clb_tipo_exhibicion.SelectedValue) == tipo_exh[j])
								{
									clb_tipo_exhibicion.SetItemChecked(i, true);
								}
							}
						}

					DataTable dt = descuento_directo.ObtenerDetalleDescuento(id_descuento);
					if (dt != null)
						dgv_descuentos.Rows.Clear();
					for (int i = 0; i < dt.Rows.Count; i++)
					{
						dgv_descuentos.Rows.Add(dt.Rows[i]["f19_tipo"], dt.Rows[i]["f19_item"], dt.Rows[i]["f19_referencia"], dt.Rows[i]["f19_descripcion"], dt.Rows[i]["f19_nit"], dt.Rows[i]["f19_proveedor"],
												dt.Rows[i]["f19_id_casa_prov"], dt.Rows[i]["f19_casa_prov"], dt.Rows[i]["f19_marca_item"], dt.Rows[i]["f19_um"], dt.Rows[i]["f19_precio"], dt.Rows[i]["f19_factor"], dt.Rows[i]["f19_pum"],
												dt.Rows[i]["f19_existencia"], dt.Rows[i]["f19_id_plan"], dt.Rows[i]["f19_plan"], dt.Rows[i]["f19_id_criterio"], dt.Rows[i]["f19_criterio"],
												dt.Rows[i]["f19_id_plan2"], dt.Rows[i]["f19_plan2"], dt.Rows[i]["f19_id_criterio2"], dt.Rows[i]["f19_criterio2"],
												dt.Rows[i]["f19_id_plan3"], dt.Rows[i]["f19_plan3"], dt.Rows[i]["f19_id_criterio3"], dt.Rows[i]["f19_criterio3"],
												dt.Rows[i]["f19_tipo_descuento"], dt.Rows[i]["f19_descuento"], dt.Rows[i]["f19_porc_proveedor"], dt.Rows[i]["f19_porc_megatiendas"],
												dt.Rows[i]["f19_precio_fin"], dt.Rows[i]["f19_pum2"], dt.Rows[i]["f19_fecha_ini_desc"], dt.Rows[i]["f19_fecha_fin_desc"], dt.Rows[i]["f19_modo_cobro"], dt.Rows[i]["f19_id_co"], dt.Rows[i]["f19_co"],
												dt.Rows[i]["f19_regional_cluster"], dt.Rows[i]["f19_ind_regional_cluster"], dt.Rows[i]["f19_cod_cluster"], dt.Rows[i]["f19_observacion"]);
					}
		...
		private void FrmEditarDescuentoDirecto_Load(object sender, EventArgs e)
		{
			Cursor = Cursors.WaitCursor;
			try
			{
				...
				max_date = dtp_fecha_fin_desc.MaxDate;
			}
			catch (Exception ex)
			{
				...
				if (rdb_item.Checked)
				{
					...
					if (existe == false)
					{
						...
						dgv_descuentos.Rows.Add(tipo, item, referencia, txt_descripcion.Text, Convert.ToString(cmb_proveedores.SelectedValue), cmb_proveedores.Text,
												...
												factor, pum, txt_existencia.Text, null, null, null, null, null, null, null, null, null, null, null, null,
		...
		private void btn_guardar_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				descuento_directo.EditarDescuento(descuento, detalles);
		...
		private void btn_confirmar_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				foreach (DataRowView item in clb_tipo_comunicacion.CheckedItems)
				{
					string[] correo_tipo_com = tipo.BuscarCorreoTipoComunicacion((int)item[0]).Split(';');

					if (!correo_tipo_com.Equals(""))
						for (int i = 0; i < correo_tipo_com.Length; i++)
						{
							string to = correo_tipo_com[i];
		...
		private void clb_tipo_comunicacion_MouseClick(object sender, MouseEventArgs e)
		{
			btn_confirmar.Enabled = false;
			btn_guardar.Enabled = true;
		}
		...
		private void btn_todos_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (rdb_plan.Checked)
				{
					...
					id_plan = cmb_plan.SelectedValue.ToString();
					plan = cmb_plan.Text;
					id_criterio = cmb_criterio.SelectedValue.ToString();
					criterio = cmb_criterio.Text;

					id_plan2 = cmb_plan2.SelectedIndex >= 0 ? cmb_plan2.SelectedValue.ToString() : null;
					plan2 = cmb_plan2.SelectedIndex >= 0 ? cmb_plan2.Text : null;
					id_criterio2 = cmb_criterio2.SelectedIndex >= 0 ? cmb_criterio2.SelectedValue.ToString() : null;
					criterio2 = cmb_criterio2.SelectedIndex >= 0 ? cmb_criterio2.Text : null;

					id_plan3 = cmb_plan3.SelectedIndex >= 0 ? cmb_plan3.SelectedValue.ToString() : null;
					plan3 = cmb_plan3.SelectedIndex >= 0 ? cmb_plan3.Text : null;
					id_criterio3 = cmb_criterio3.SelectedIndex >= 0 ? cmb_criterio3.SelectedValue.ToString() : null;
					criterio3 = cmb_criterio3.SelectedIndex >= 0 ? cmb_criterio3.Text : null;
				}
		...
		private void clb_tipo_exhibicion_MouseClick(object sender, MouseEventArgs e)
		{
			btn_confirmar.Enabled = false;
			btn_guardar.Enabled = true;
		}
		...
		private void cargarArchivoToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				if (openfile.ShowDialog() == DialogResult.OK)
				{
					...
					string[] regional_conf = section.Settings["regional"].Value.Split(',');//LISTA DE REGIONALES QUE ESTAN EN EL ARCHIVO DE CONFIGURACION.
					string[] co_conf = section.Settings["co"].Value.Split(',');//LISTA DE CO QUE ESTAN EN EL ARCHIVO DE CONFIGURACION.
					...
					int c = 0;
						foreach (var fila_hoja_items in filas_hoja_items)
						{
							...
							List<string> list_regional = new List<string>();
							for (int i = 0; i < regional_conf.Length; i++)
							{
								for (int j = 0; j < co_regional.Length; j++)
								{
									if (co_regional[j].ToUpper().Trim().Equals(regional_conf[i]))
									{
										list_regional.Add(regional_conf[i]);
									}
								}
							}

							List<string> list_co = new List<string>();//AQUI SE TRAE LOS CO QUE ENCONTRO EN LA CELDA.
							for (int i = 0; i < co_conf.Length; i++)
							{
								for (int j = 0; j < co_regional.Length; j++)
								{
									if (co_regional[j].ToUpper().Trim().Equals(co_conf[i]))
									{
										list_co.Add(co_conf[i]);
									}
								}
							}
							...
							foreach (string r in list_co)
							{
								...
								foreach (var row_co in rows_co)
								{
									string id_co = Convert.ToString(row_co.Cell("B").Value).Trim();//EL ID DEL CO.
									string desc_co = Convert.ToString(row_co.Cell("C").Value).Trim();//LA DESCRIPCION DEL CO.
									string regional = Convert.ToString(row_co.Cell("A").Value).Trim();
									if (desc_co.Equals(r))
									{
		...
		private void cmb_modo_cobro_MouseClick(object sender, MouseEventArgs e)
		{
			btn_confirmar.Enabled = false;
		}
