...
{
	public partial class FrmReporteComercial : Form
	{
		DataTable dt_detalle = null;

		public FrmReporteComercial()
		{
			InitializeComponent();
		}
		...
		private void CrearReporteComercial(string fecha_ini, string fecha_fin, int tipo_dinamica, bool consecutivo_retro, bool consecutivo_descuento, int consecutivo)
		{
			...
			dgv_reporte.DataSource = reportes.CrearReporteComercial(fecha_ini, fecha_fin, tipo_dinamica, consecutivo_retro, consecutivo_descuento, consecutivo);
		}
		...
		private void VerDetalleDescuento(int consecutivo)
		{
			...
			col_modo_cobro.DataPropertyName = "f19_modo_cobro";
			col_descuento.DataPropertyName = "f19_descuento";
			col_tipo_desc.DataPropertyName = "tipo_descuento";
			col_asume_prov.DataPropertyName = "f19_porc_proveedor";
			col_asume_mega.DataPropertyName = "f19_porc_megatiendas";
			...
			Reportes reportes = new Reportes();
			dt_detalle=reportes.VerDetalleReporteComercial(consecutivo);
			dgv_detalle.DataSource = dt_detalle;
		}

		private void FrmReporteComercial_Load(object sender, EventArgs e)
		{
			try
			{
				...
				if (Usuarios.Permisos.Contains("030301"))
				{
		...
		private void btn_buscar_Click(object sender, EventArgs e)
		{
			try
			{
				...
				CrearReporteComercial(dtp_fecha_ini.Value.Date.ToString("yyyyMMdd"), dtp_fecha_fin.Value.Date.ToString("yyyyMMdd"), tipo_dinamica, chk_retro.Checked, chk_descuento.Checked, consecutivo);
			...
			try
			{
				if (e.RowIndex >= 0 && dgv_reporte.Columns[e.ColumnIndex].Name == "col_detalle")
				{
					...
					/*foreach (DataGridViewRow row in dgv_detalle.Rows)
					{
						...
						{
							row.Cells[0].Style.Font = new System.Drawing.Font("Arial", 8, FontStyle.Underline);
							row.Cells["col_item"].Style.ForeColor = System.Drawing.Color.Blue;
						}
					}*/
				}
				if (e.RowIndex >= 0 && dgv_reporte.Columns[e.ColumnIndex].Name == "col_cargado")
				{
					if (Usuarios.Permisos.Contains("030302"))
					{
						if (MessageBox.Show("¿Confirma marcar como Cargado este descuento?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question).Equals(DialogResult.Yes))
						{
							int consecutivo = Convert.ToInt32(dgv_reporte["col_consecutivo", e.RowIndex].Value);
							DescuentoDirecto descuento_directo = new DescuentoDirecto();
							descuento_directo.CambiarEstadoDescuento(consecutivo, 2);
		...
		private void btn_exportar_Click(object sender, EventArgs e)
		{
			try
			{
				_saveFileDialog.FileName = "Reporte comercial";
		...
		{
			if (Usuarios.Permisos.Contains("03030101"))
			{
				Cursor = Cursors.WaitCursor;
				try
				{
